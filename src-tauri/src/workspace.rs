...
pub struct WorkspaceFile {
    ...
    /// Only consulted when the per-workspace override is on in Settings;
    /// otherwise the app-wide target applies. None means "inherit".
    ///
    /// There is no catch-all field on this struct, so anything missing here is
    /// dropped from the file on the next save - hence the explicit entry.
    #[serde(default, skip_serializing_if = "Option::is_none")]
